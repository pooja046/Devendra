...
public class CustomPagerAdapter extends PagerAdapter {
    ...
    public CustomPagerAdapter(Context context, ArrayList<GamesDTO> gamesDTOs) {
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        Picasso.with(mContext).load(gamesDTOs.get(position).getImage()).placeholder(R.drawable.white_box).error(R.drawable.white_box).into(image_new);
        image_new.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                try {
                    MainActivity.vUrl = gamesDTOs.get(position).getLinkDTO().getUrl();
                } catch (Exception e) {
