        ...
        protected void onPostExecute(JSONObject jsonObject) {
            ...
            try {
                ...
                if (vStatus.trim().equals("Success")) {
                    ...
                    JSONArray product_attribute = cart_dataObject
                            .getJSONArray("product_attribute");
                    JSONObject product_attribute_details = cart_dataObject
                            .getJSONObject("product_attribute_details");
                    if (id_product.length() > 0) {
                        ...
                        for (int i = 0; i < id_product.length(); i++) {
                            ...
                            try {
                                JSONArray product_attribute_array = product_attribute_details.getJSONArray(product_attribute.getString(i));
                                try {
                                    if (product_attribute_array.getJSONObject(0).getString("attribute_name").equals("Color")) {
                                        cartsDTO.setColor_name(product_attribute_array.getJSONObject(0).getString("value"));
                                    }
                                } catch (Exception e) {

                                }
                                try {
                                    if (product_attribute_array.getJSONObject(0).getString("attribute_name").equals("Size")) {
                                        cartsDTO.setSize(product_attribute_array.getJSONObject(0).getString("value").split(" ")[0].replace("Short", "S").replace("Medium", "M").replace("Long", "L"));
                                    }
                                } catch (Exception e) {

                                }
                                try {
                                    if (product_attribute_array.getJSONObject(1).getString("attribute_name").equals("Size")) {
                                        cartsDTO.setSize(product_attribute_array.getJSONObject(1).getString("value").split(" ")[0].replace("Short", "S").replace("Medium", "M").replace("Long", "L"));
                                    }
                                } catch (Exception e) {
